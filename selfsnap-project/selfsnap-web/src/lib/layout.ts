...
export type LayoutMode = "grid-2x2" | "strip-3";

export function compute3StripSlots(
  canvasW: number,
  canvasH: number,
  opts?: {
    outerPad?: number;
  ...
  // Each slot: full width, height = width * 3/4 (landscape 4:3)
  const shotW = usableW;
  const totalGaps = gap * 2;
  const shotH = (usableH - totalGaps) / 3;

  const gridLeft = outerPad;
  const gridTop = outerPad + headerH;

  return [
    { x: gridLeft, y: gridTop, w: shotW, h: shotH },
    { x: gridLeft, y: gridTop + shotH + gap, w: shotW, h: shotH },
    { x: gridLeft, y: gridTop + shotH * 2 + gap * 2, w: shotW, h: shotH },
  ];
}
