...
async function loadFrames() {
  ...
  try {
    ...
    if (!stored.frameUrl) throw new Error("Aucun événement sélectionné. Revenez à la page d'accueil.");

    // Frame fixe chargée depuis la Landing via sessionStorage
    frames.value = [{ s3Key: "event", name: "Frame événement", url: stored.frameUrl }];
    selectedFrameKey.value = "event";
  } catch (e: any) {
