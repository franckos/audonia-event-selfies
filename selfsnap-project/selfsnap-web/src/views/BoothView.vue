          ...
          <div class="mt-4 flex flex-col items-center justify-center gap-3">
            ...
            <div class="flex gap-2">
              <button
                ...
                "
                :disabled="isRunning"
                @click="layout = 'grid-2x2'"
                title="4 photos en grille 2×2"
              >
                ▦ 2×2
              </button>
              <button
                ...
                "
                :disabled="isRunning"
                @click="layout = 'strip-3'"
                title="3 photos en strip vertical"
              >
                ☰ Strip 3
              </button>
            </div>
            ...
            <div
              v-if="isRunning"
              class="text-center text-sm text-black-500 font-medium"
            >
...
const layout = ref<LayoutMode>(settings.layout ?? "grid-2x2");
const shotCount = computed(() => (layout.value === "strip-3" ? 3 : 4));

const slotsPx = computed(() => {
  if (layout.value === "strip-3") {
    return compute3StripSlots(FRAME_W, FRAME_H, {
  ...
  return compute4GridSlots(FRAME_W, FRAME_H, {
    outerPad: LAYOUT.outerPad,
    gap: LAYOUT.gap,
...
async function captureOneShot(_: number): Promise<string> {
  ...
  // Capture at the exact slot ratio to avoid zoom/crop mismatch in ResultView.
  // Slot dimensions are derived from the same constants used in compose().
  const outW = 1080;
  let outH: number;
  if (layout.value === "strip-3") {
    // Mirror compute3StripSlots: shotH = (usableH - gap*2) / 3
    const usableH = FRAME_H - LAYOUT.outerPad * 2 - LAYOUT.headerH - LAYOUT.footerH;
    const slotH = (usableH - LAYOUT.gap * 2) / 3;
    const slotW = FRAME_W - LAYOUT.outerPad * 2;
    outH = Math.round((outW / slotW) * slotH);
  } else {
    outH = 1440; // grid-2x2 portrait 3:4
  }
...
async function startSequence() {
  ...
  try {
    for (let i = 0; i < shotCount.value; i++) {
    ...
    const stored = JSON.parse(
      sessionStorage.getItem("selfsnap.settings") ?? "{}",
    );
    sessionStorage.setItem(
      "selfsnap.settings",
      JSON.stringify({ ...stored, layout: layout.value }),
    );
